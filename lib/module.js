class Module {
  ...
  /**
   * Return you the Status of registration of the module
   * @param {string} moduleId
   * @returns {string} The status of registration of the module
   */
  async getStatusRegisterModule(moduleId) {
    return new Promise(async (resolve, reject) => {
      const year = await this.findYearModule(parseInt(moduleId.substr(6, 1)));
      const city = await this.intranetApi.user.getCity();
      const login = await this.intranetApi.user.getEpitechEmail();
      this.intranetApi
        .get(`/module/${year}/${moduleId}/${city}-${moduleId.substr(6, 1)}-1/registered/`)
        .then((response) => {
          response = response.find(module => module.login === login)
          if (response)
            resolve("Registered");
          else 
