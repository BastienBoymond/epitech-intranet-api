      ...
      this.intranetApi
        ...
        .then(async (response) => {
          ...
          for (const activity of activities) {
            ...
            } else if (activity.type_title == "Talk" || activity.type_title == "Meetup") {
              let evnt = activity.events;
              let dataevnt = evnt.length;
              for (let j = 0; j < dataevnt; j++) {
                if (evnt[j].assistants) {
          ...
          if (!details) {
            ...
            resolve({xp: xp, details: {limitExperience: limitExperience, limitOrganizationHubTalk: limitOrganizationHubTalk, limitParticipationHubTalk: limitParticipationHubTalk, limitOrganizationWorkshop: limitOrganizationWorkshop, limitParticipationWorshop: limitParticipationWorshop, hackathonsOrganisation: hackathonsOrganisation, hackathonsParicipation: hackathonsParicipation, hubProjects: hubProjects}});
          }
        });
    }).catch((error) => {
      reject(error);
